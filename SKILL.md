...
| 字段 | 类型 | 必需 | 说明 |
|------|------|------|------|
| `input` | string | ✅ | 源文件绝对路径，或 M3U8 URL（`isM3U8=true` 时） |
| `isM3U8` | bool | | `true` 时 `input` 为 M3U8 URL，脚本自动下载并生成 sub-M3U8 |
| `member` | string | | 成员名称，如 `谭思慧`（口袋48录播模式时提供，用于输出目录） |
| `broadcastTime` | string | | 开播时间，如 `2026-06-20~23.20.40`（口袋48录播模式时提供，用于输出目录） |
| `outDir` | string | | 输出根目录，默认 `process.cwd()` |
...
**口袋48模式**（`member` + `broadcastTime` 指定时）：输出到 `{outDir}/{member}/{broadcastTime}/`：
...
有两种方式指定源文件。LLM 根据用户输入自动判断采用哪种。

#### 方式 A：传统模式

用户直接提供本地音视频文件路径，可指定输出格式和编码器。
...
用户指定成员和开播时间（或通过上下文推断），自动查询口袋48录播回放的 M3U8 地址作为源。

**判断依据**：LLM 从以下来源提取 `{成员名}` + `{开播时间}`：

| 信息来源 | 说明 | 示例 |
|---------|------|------|
| 对话内容 | 用户直接提及 | "谭思慧6月20号那场直播" |
| 切片本文件名 | 文件名包含成员/时间 | `谭思慧_2026-06-20~23.20.40.txt` |
| 父/祖父目录名 | 目录路径包含成员/时间 | `谭思慧/2026-06-20~23.20.40/_clip_record.txt` |

**三种结果**：
- ✅ **成员 + 时间都找到** → 进入口袋48模式工作流
- ⚠️ **只找到一项** → 询问用户补充缺失的信息
- ❌ **都没找到** → 回退到传统模式（要求用户提供本地文件路径）

> **注意**：开播时间格式统一为 `YYYY-MM-DD~HH.MM.SS`（如 `2026-06-20~23.20.40`），与 pocket48-replays skill 保持一致。

### Step 1.5 (口袋48模式): 查询录播回放

用 pocket48-replays skill 的脚本查询唯一匹配的录播：

**1. 解析成员名 → userId**

```bash
node <pocket48_skill_dir>/scripts/download_pocket48_lives.mjs \
  --member "谭思慧" \
  --since "2026-06-20 23:20:00" \
  --until "2026-06-20 23:25:00" \
  --info-only
```

先用 `--info-only` 查看录播信息。如果时间精确，应只返回一条结果。

**2. 展示给用户确认**

展示录播的详细信息（时间、标题、时长等），必须获得用户 **明确确认** 后才能继续。

**3. 获取 M3U8 地址**

确认后，用 `--json` 参数获取完整元数据，从中提取 `playStreamPath`（M3U8 地址）。
...
node <pocket48_skill_dir>/scripts/download_pocket48_lives.mjs \
  ...
  --since "2026-06-20 23:20:00" \
  --until "2026-06-20 23:25:00" \
  --json 2>/dev/null | grep -o '"playStreamPath":"[^"]*"' | head -1
```

或直接从 `--info-only` 的输出中读取 `M3U8:` 行。

> **注意**：如果用户提供了现成的 M3U8 URL，跳过此步骤直接构造 JSON。
...
LLM 将理解后的信息构造成 JSON。如果是导入切片本文件，先确认提取结果后再传给脚本。
...
```json
{
  "input": "https://...playback.m3u8",
  "isM3U8": true,
  "member": "谭思慧",
  "broadcastTime": "2026-06-20~23.20.40",
  "outDir": "/Users/cbj/Documents/48",
  "defaultFormat": "mp4",
  "clips": [
    { "name": "片段A", "start": "00:17:48", "end": "00:20:24" },
    { "name": "片段B", "start": "01:07:13", "end": "01:11:25" }
  ]
}
```
...
2. ffprobe 探测源文件音视频编码（M3U8 模式下失败则默认 H.264 + AAC）
3. 根据输出格式 + 编码兼容性 + accurate 标志，为每个 clip 选择最优策略
4. 逐条执行 ffmpeg 命令
   - **传统模式**：`-i input -ss start -to end`（output seeking）
   - **口袋48模式**：先解析 M3U8 生成本地 sub-M3U8（只含覆盖 clip 范围的少数分片），再对本地文件用 output seeking
5. 生成 `_clip_record.txt` 到相应输出目录
6. 输出结果汇总
...
| 异常 | 行为 |
|------|------|
| ffprobe 探测失败（传统模式） | 终止该批次，打印错误日志 |
| ffprobe 探测失败（口袋48 M3U8 模式） | 默认 H.264 + AAC，继续执行 |
| ffmpeg 执行失败 | 跳过该 clip，记录到 _clip_record.txt 失败段 |
| 本地文件不存在 | 打印错误，跳过 |
| M3U8 URL 不可达 | ffmpeg 超时后报错，记录到失败段 |
...
- 混合格式场景下，`_clip_record.txt` 只生成一份，放在输出根目录
- 口袋48 M3U8 模式：脚本自动解析 M3U8 分片列表，生成只含所需分片引用的本地 sub-M3U8，再对 sub-M3U8 做 output seeking，确保时长精确的同时只拉取必要分片
- 口袋48模式查询 API 仅在工作流中做一次，ffmpeg 直接读 CDN 分片，无封 IP 风险
